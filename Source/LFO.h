...
public:
	...
	float minimaxSin(float x1) {
		...
		float x2 = x1 * x1;
		return x1 * (0.99999999997884898600402426033768998f + x2 * (-0.166666666088260696413164261885310067f + x2 * (0.00833333072055773645376566203656709979f + x2 * (-0.000198408328232619552901560108010257242f + x2 * (float)(2.75239710746326498401791551303359689e-6 - 2.3868346521031027639830001794722295e-8 * x2)))));
	}

	LFOValuesByPhase calculateValuesForPhases() {
		// check whether any phase needs to be wrapped, and if so mark it as flipped
		// and wrap it. If needed, we'll use the info to trigger a new random value.
		...
		case 0: // sine
			...
			values.minusQuarterPhaseValue = minimaxSin(phases.minusQuarterPhase);
			values.halfPhaseValue = minimaxSin(phases.halfPhase);
			break;
		case 1: // triangle
			values.mainPhaseValue = getTriangleValueForPhase(phases.mainPhase);
			values.quarterPhaseValue = getTriangleValueForPhase(phases.quarterPhase);
			values.minusQuarterPhaseValue = getTriangleValueForPhase(phases.minusQuarterPhase);
			values.halfPhaseValue = getTriangleValueForPhase(phases.halfPhase);
			break;
		case 2: // saw up
			values.mainPhaseValue = phases.mainPhase / juce::MathConstants<float>::pi;
			values.quarterPhaseValue = phases.quarterPhase / juce::MathConstants<float>::pi;
			values.minusQuarterPhaseValue = phases.minusQuarterPhase / juce::MathConstants<float>::pi;
			values.halfPhaseValue = phases.halfPhase / juce::MathConstants<float>::pi;
			break;
		case 3: // saw down
			values.mainPhaseValue = -phases.mainPhase / juce::MathConstants<float>::pi;
			values.quarterPhaseValue = -phases.quarterPhase / juce::MathConstants<float>::pi;
		...
		case 4: // square
			...
			values.quarterPhaseValue = phases.quarterPhase < 0 ? -1.0f : 1.0f;
			values.minusQuarterPhaseValue = phases.minusQuarterPhase < 0 ? -1.0f : 1.0f;
			values.halfPhaseValue = phases.halfPhase < 0 ? -1.0f : 1.0f;
			break;
		case 5: // random
			if (mainPhaseHasFlipped) {
				currentRandomMain = myRand.nextFloat() * 2.0f - 1.0f; // save for next X samples
			...
			if (minusQuarterPhaseHasFlipped) {
				currentRandomMinusQuarter = myRand.nextFloat() * 2.0f - 1.0f;
	...
	LFOValuesByPhase getCurrentValues() {
		if (isBipolar) {
			...
			values.minusQuarterPhaseValue = depth * values.minusQuarterPhaseValue;
			values.halfPhaseValue = depth * values.halfPhaseValue;
		} 
		else
		{
			values.mainPhaseValue = depth * (values.mainPhaseValue + 1.0f) / 2.0f;
			values.quarterPhaseValue = depth * (values.quarterPhaseValue + 1.0f) / 2.0f;
			values.minusQuarterPhaseValue = depth * (values.minusQuarterPhaseValue + 1.0f) / 2.0f;
			values.halfPhaseValue = depth * (values.halfPhaseValue + 1.0f) / 2.0f;
		}
		return values;
	}
	...
	void initialize() {
		...
		mainPhaseHasFlipped = false; quarterPhaseHasFlipped = false; minusQuarterPhaseHasFlipped = false; halfPhaseHasFlipped = false;
	}
...
private:
	...
	bool mainPhaseHasFlipped{ false }, quarterPhaseHasFlipped{ false }, minusQuarterPhaseHasFlipped{ false }, halfPhaseHasFlipped{ false };
	bool isBipolar{ true };
