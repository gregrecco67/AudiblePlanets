...
float64x2_t	load2f64a (const double *ptr) noexcept
{
...
float64x2_t	load2f64u (const double *ptr) noexcept
{
	return vreinterpretq_f64_u8 (vld1q_u8 (
		reinterpret_cast <const uint8_t *> (ptr)
	));
}
...
void	storea (double *ptr, float64x2_t x) noexcept
{
	vst1q_f64 (ptr, x);
}
...
void	storeu (double *ptr, float64x2_t x) noexcept
{
	vst1q_u8 (reinterpret_cast <uint8_t *> (ptr), vreinterpretq_u8_f64 (x));
}
