...
APLevelMeter::APLevelMeter (const gin::LevelTracker& l, juce::NormalisableRange<float> r) :
    tracker (l),
    range (r)
{
    startTimerHz (30);
...
void APLevelMeter::paint (juce::Graphics& g)
{
    ...
    if ( tracker.getClip () )
    {
        ...
        g.fillRect ( getLocalBounds ().toFloat ().withTrimmedLeft ( rc.getWidth () * 0.95f ) );
    }
}


void APLevelMeter::timerCallback()
{
    repaint();
}
